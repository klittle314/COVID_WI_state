...
S_update_upper <- function(S_current, k_value, current_count){
  
  S_upper <- max(0,current_count - k_value + S_current, na.rm=TRUE)
  
  return(S_upper)
}


S_update_lower <- function(S_current, k_value, current_count){
  S_lower <- min(0,current_count - k_value + S_current, na.rm = TRUE)
...
make_series <- function(n,nrep,k_test,Po_mean,N_day) {
  ...
  for(j in 1:nrep) {
    ...
    for(i in seq_along(Poisson_values)) {
      cusum[i] <- S_update_upper(S_current,k_value = k_test,current_count = Poisson_values[i])
